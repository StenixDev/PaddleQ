import { BarChart3, Copy, Save, X } from 'lucide-react';
...
export default function StatsDashboard() {
  ...
  const [shareDate, setShareDate] = useState(state.shareMeta?.date || new Date().toISOString().slice(0, 10));
  const rows = useMemo(() => {
          ...
          <div>
            ...
            {state.shareMeta ? (
              <p className="text-sm text-slate-500">
                {state.shareMeta.title} {state.shareMeta.date ? `- ${state.shareMeta.date}` : ''}
              </p>
            ) : null}
          </div>
        </div>
...
function SaveDialog({ title, date, link, onTitleChange, onDateChange, onClose, onSave }) {
        ...
        <div className="grid gap-3">
          ...
          {link ? (
            <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
              <div className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Shareable link</div>
              ...
              <p className="mt-2 text-sm text-slate-500">{copied ? 'Copied.' : 'Anyone with this link can view the saved results.'}</p>
            </div>
          ) : null}
        </div>
      </div>
