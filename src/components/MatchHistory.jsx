...
import { exportDuprCsv } from '../utils/csv.js';
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-slate-950">Match History</h2>
        </div>
      ...
      <div className="space-y-3">
        {state.history.length ? (
          state.history.map((match, index) => (
            <article key={match.id} className="rounded-md border border-slate-200 bg-slate-50 p-3">
              <div className="mb-2 flex items-center justify-between text-xs text-slate-500">
                <span>Game {state.history.length - index} · Court {match.court}</span>
              ...
              <label className="mb-3 flex flex-wrap items-center gap-2 text-sm text-slate-600">
                <span className="text-xs font-bold uppercase tracking-wide text-slate-500">Winner</span>
                <select
                  value={match.winner}
                  onChange={(event) =>
                    dispatch({ type: 'updateHistoryWinner', matchId: match.id, winner: event.target.value })
                  }
              ...
              <div className="grid gap-2 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
                <HistoryTeam
                  ...
                  score={match.scoreA}
                  onScoreChange={(value) =>
                    dispatch({ type: 'updateHistoryScore', matchId: match.id, field: 'scoreA', value })
                  }
                />
                ...
                <HistoryTeam
                  ...
                  score={match.scoreB}
                  onScoreChange={(value) =>
                    dispatch({ type: 'updateHistoryScore', matchId: match.id, field: 'scoreB', value })
                  }
                />
              </div>
            </article>
...
function HistoryTeam({ players, won, score, onScoreChange }) {
  return (
    <div className={`rounded-md border p-2 ${won ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className={`text-xs font-bold uppercase tracking-wide ${won ? 'text-emerald-700' : 'text-slate-400'}`}>
          {won ? 'Winner' : 'Loser'}
        </span>
